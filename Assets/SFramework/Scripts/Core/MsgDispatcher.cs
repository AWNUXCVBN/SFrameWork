...
namespace SFramework
{
    ...
    public static class MsgDispatcher
    {
        ...
        /// 每个消息名字维护一组消息捕捉器。
        /// </summary>
        static Dictionary<MsgChannel, Dictionary<string, List<MsgHandler>>> mMsgHandlerDict = new Dictionary<MsgChannel, Dictionary<string, List<MsgHandler>>>();
        ...
        public static void RegisterGlobalMsg(this IMsgReceiver self, string msgName, VoidDelegate.WithParams callback)
        {
            ...
            // 添加消息通道
            if (!mMsgHandlerDict.ContainsKey(MsgChannel.Global))
            {
                mMsgHandlerDict[MsgChannel.Global] = new Dictionary<string, List<MsgHandler>>();
            }

            // 略过
            if (!mMsgHandlerDict[MsgChannel.Global].ContainsKey(msgName))
            {
                mMsgHandlerDict[MsgChannel.Global][msgName] = new List<MsgHandler>();
            }

            // 看下这里
            var handlers = mMsgHandlerDict[MsgChannel.Global][msgName];
        ...
        public static void RegisterMsgByChannel(this IMsgReceiver self, MsgChannel channel, string msgName, VoidDelegate.WithParams callback)
        {
            ...
            {
                mMsgHandlerDict[channel] = new Dictionary<string, List<MsgHandler>>();
            }

            // 略过
            if (!mMsgHandlerDict[channel].ContainsKey(msgName))
            {
                mMsgHandlerDict[channel][msgName] = new List<MsgHandler>();
            }
